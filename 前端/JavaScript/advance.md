...
| ------------------------- | ------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `arr.map(callbackFn)`     | Creates a new array with the results of calling `callbackFn` on each element in `arr`.            | `js let languages = ["Python", "Java", "C++", "PHP", "JS"]; let result = languages.map((lang) => lang.toUpperCase()); console.log(result); // ['PYTHON', 'JAVA', 'C++', 'PHP', 'JS'] `                      |
| `arr.find(callbackFn)`    | Returns the **first element** in `arr` that satisfies `callbackFn`, or `undefined` if none match. | `js const languages = [{ name: "Python", rating: 9.5 }, { name: "Java", rating: 9.4 }]; let result = languages.find((lang) => lang.rating > 9.4); console.log(result); // { name: 'Python', rating: 9.5 } ` |
| `arr.filter(callbackFn)`  | Filters `arr` based on `callbackFn` and returns a new **subset array**.                           | `js let result = languages.filter((lang) => lang.rating >= 9.2); console.log(result); `                                                                                                                     |
| `arr.some(callbackFn)`    | Checks if **at least one** element in `arr` satisfies `callbackFn`, returning `true` or `false`.  | `js let result = languages.some((lang) => lang.rating >= 9.2); console.log(result); // true `                                                                                                               |
| `arr.every(callbackFn)`   | Checks if **all elements** in `arr` satisfy `callbackFn`, returning `true` or `false`.            | `js let result = languages.every((lang) => lang.rating >= 9.0); console.log(result); // true `                                                                                                              |
| `arr.forEach(callbackFn)` | Executes `callbackFn` on **each element** in `arr` without returning a new array.                 | `js let languages = ["Python", "Java", "C++"]; languages.forEach((lang) => console.log(lang)); // Outputs: Python, Java, C++ `                                                                              |

# 進階 Array Methods

| 方法                      | 描述                                                                            | 範例                                                                                                                                                                                                        |
| ------------------------- | ------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `arr.map(callbackFn)`     | 創建一個新 array，其中填充了在 `arr` 中的每個元素上調 `callbackFn` 的結果       | `js let languages = ["Python", "Java", "C++", "PHP", "JS"]; let result = languages.map((lang) => lang.toUpperCase()); console.log(result); // ['PYTHON', 'JAVA', 'C++', 'PHP', 'JS'] `                      |
| `arr.find(callbackFn)`    | 返回 `arr` 中滿足 `callbackFn` 條件的**第一個元素**，如果沒有則回傳 `undefined` | `js const languages = [{ name: "Python", rating: 9.5 }, { name: "Java", rating: 9.4 }]; let result = languages.find((lang) => lang.rating > 9.4); console.log(result); // { name: 'Python', rating: 9.5 } ` |
| `arr.filter(callbackFn)`  | 過濾 `arr` 中符合 `callbackFn` 條件的元素，回傳一個新的**子陣列**               | `js let result = languages.filter((lang) => lang.rating >= 9.2); console.log(result); `                                                                                                                     |
| `arr.some(callbackFn)`    | 測試 `arr` 是否**至少有一個**元素通過 `callbackFn` 條件，回傳 `true` 或 `false` | `js let result = languages.some((lang) => lang.rating >= 9.2); console.log(result); // true `                                                                                                               |
| `arr.every(callbackFn)`   | 測試 `arr` 是否**所有元素**都符合 `callbackFn` 條件，回傳 `true` 或 `false`     | `js let result = languages.every((lang) => lang.rating >= 9.0); console.log(result); // true `                                                                                                              |
| `arr.forEach(callbackFn)` | 針對 `arr` 中的**每個元素**執行 `callbackFn`，不會回傳新陣列                    | `js let languages = ["Python", "Java", "C++"]; languages.forEach((lang) => console.log(lang)); // 逐個輸出 Python, Java, C++ `                                                                              |
